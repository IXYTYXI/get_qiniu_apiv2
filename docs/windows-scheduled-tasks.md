...
默认同时处理最多 3 场，下载和转码均可并行；附件上传按场次串行，应用身份素材请求共享 250 ms 间隔。范围补采仍逐日执行，不会同时启动多个日期的并发池。每日任务不加参数即使用 3 场；若需降低占用，在任务的“添加参数”末尾增加 `-Workers 1` 或 `-Workers 2`。范围脚本也支持 `-Workers 3`。
